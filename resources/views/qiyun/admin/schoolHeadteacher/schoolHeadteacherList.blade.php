...
@section('content')
        ...
        <div class="nav-search" id="nav-search">
            <form action="{{url('admin/school/schoolHeadteaherList')}}" method="get" class="form-search">
                <select name="type" id="form-field-1">
                    ...
                    <option value="5" @if($data->type == 5) selected @endif>班主任</option>
    ...
    <div class="page-content">
        ...
        <a href="{{url('admin/school/addSchoolHeadteaher')}}" class="btn btn-xs btn-info">
            <i class="icon-ok bigger-110">添加</i>
        </a>
        @endpermission

        @permission('posts.add')
        <form action="{{url('admin/excel/schoolHeadteacherImport')}}" method="post" enctype="multipart/form-data" style="float: right;">
            <input type="submit" class="btn btn-xs btn-info" value="导入班主任信息" >
            <input type="file" name="excel" style="float:right">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
        </form>
        <a href="{{url('admin/excel/schoolHeadteacherTemplate')}}" class="btn btn-xs btn-info" style="float: right;margin-right: 10px;">
            下载导入模板
        </a>
        @endpermission
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="table-responsive">
                            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                ...
                                <tbody>
                                  <form action="" method="post">
                                      <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                      @foreach($data as $schoolHeadteacher)
                                          <tr>
                                              {{--<td class="center">--}}
                                                  {{--<label>--}}
                                              ...
                                              {{--</td>--}}
                                              <td>
                                                  <a href="#">{{$schoolHeadteacher->id}}</a>
                                              </td>
                                              <td>{{$schoolHeadteacher->schoolName}}</td>
                                              <td>{{$schoolHeadteacher->gradeName}}</td>
                                              <td>{{$schoolHeadteacher->classname}}</td>
                                              <td>{{$schoolHeadteacher->realname}}</td>
                                              <td>{{$schoolHeadteacher->status ? '激活':'锁定'}}</td>
                                              <td>{{$schoolHeadteacher->created_at}}</td>
                                              <td>{{$schoolHeadteacher->updated_at}}</td>
                                              ...
                                              <td>
                                                  <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                      ...
                                                      <a href="{{url('admin/school/editSchoolHeadteaher/'.$schoolHeadteacher->id)}}" class="btn btn-xs btn-info">
                                                          <i class="icon-edit bigger-120"></i>
                                                      </a>
                                                      @endpermission

                                                      @permission('posts.del')
                                                      <a href="{{url('admin/school/delSchoolHeadteaher/'.$schoolHeadteacher->id)}}" class="btn btn-xs btn-danger" onclick="return confirm('删除后无法找回,确定要删除吗?');">
                            ...
                            {{--<a href="{{url('admin/excel/schoolHeadteacherExport')}}" class="btn btn-xs btn-info" style="float:right">--}}
                                {{--<i class="icon-ok bigger-110">导出班主任信息</i>--}}
                            {{--</a>--}}
                            <form action="{{url('admin/excel/schoolHeadteacherExport')}}" method="post" style="float: right;">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <input type="hidden" name="export" value="{{$export}}">
                                <input type="submit" class="btn btn-xs btn-info" value="导出班主任信息">
                            </form>
