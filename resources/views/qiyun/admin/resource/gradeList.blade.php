...
@section('content')
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <div class="nav-search" id="nav-search">
            <form action="{{url('admin/resource/gradeList')}}" method="get" class="form-search">
                <select name="type" id="form-field-1">
                    ...
                    <option value="6" @if($data->type == 6) selected @endif>册别名称</option>
                </select>
                                  ...
                                  <form action="">
                                      @foreach($data as $grades)
                                          <tr>
                                              ...
                                              <td>{{$grades->sectionName}}</td>
                                              <td>{{$grades->subjectName}}</td>
                                              <td>{{$grades->editionName}}</td>
                                              <td>{{$grades->gradeName}}</td>


                                              <td>
                                                  <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                      <!-- 编辑 -->
                                                      <a href="{{url('admin/resource/gradeEdit/'.$grades->id)}}" class="btn btn-xs btn-info">
                                                          <i class="icon-edit bigger-120"></i>
                                                      </a>
                                                      <!-- 删除 -->
                                                      <a href="{{url('admin/resource/gradeDel/'.$grades->id)}}" class="btn btn-xs btn-danger" onclick="return confirm('删除后将无法找回,确定要删除吗?');">
