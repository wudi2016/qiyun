...
@section('content')
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <div class="nav-search" id="nav-search">
            <form action="{{url('admin/school/schoolTermList')}}" method="get" class="form-search">
                <select name="type" id="form-field-1">
                    ...
                    <option value="4" @if($data->type == 4) selected @endif>学期名称</option>
    ...
    <div class="page-content">
        ...
        <a href="{{url('admin/school/addSchoolTerm')}}" class="btn btn-xs btn-info">
            <i class="icon-ok bigger-110">添加</i>
        </a>
        @endpermission

        @permission('base.create')
        <form action="{{url('admin/excel/schoolTermImport')}}" method="post" enctype="multipart/form-data" style="float: right;">
            <input type="submit" class="btn btn-xs btn-info" value="导入学期信息">
            <input type="file" name="excel" style="float: right;">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            {{--<i class="icon-ok bigger-110" >导入学校信息</i>--}}
        </form>
        <a href="{{url('admin/excel/schoolTermTemplate')}}" class="btn btn-xs btn-info" style="float: right;margin-right: 10px;">
            下载导入模板
        </a>
        @endpermission
                            ...
                            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                ...
                                <tbody>
                                  <form action="" method="post">
                                      <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                      @foreach($data as $schoolTerm)
                                          <tr>
                                              {{--<td class="center">--}}
                                                  {{--<label>--}}
                                                      {{--<input type="checkbox" class="ace" name="id[]" value="{{$schoolTerm->id}}"/>--}}
                                                      {{--<span class="lbl"></span>--}}
                                                  {{--</label>--}}
                                              {{--</td>--}}

                                              <td>
                                                  <a href="#">{{$schoolTerm->id}}</a>
                                              </td>
                                              <td>{{$schoolTerm->schoolName}}</td>
                                              <td>{{$schoolTerm->reportName}}</td>
                                              <td>{{$schoolTerm->termName}}</td>
                                              <td>{{$schoolTerm->startTime}}</td>
                                              <td>{{$schoolTerm->endTime}}</td>
                                              <td>{{$schoolTerm->reportTitle}}</td>
                                              <td>{{$schoolTerm->reportBody}}</td>
                                              <td>{{$schoolTerm->reportTermTime}}</td>
                                              <td>{{$schoolTerm->status ? '激活':'锁定'}}</td>
                                              <td>{{$schoolTerm->created_at}}</td>
                                              <td>{{$schoolTerm->updated_at}}</td>
                                              ...
                                              <td>
                                                  <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                      ...
                                                      <a href="{{url('admin/school/editSchoolTerm/'.$schoolTerm->id)}}" class="btn btn-xs btn-info">
                                                          <i class="icon-edit bigger-120"></i>
                                                      </a>
                                                      @endpermission

                                                      @permission('base.remove')
                                                      <a href="{{url('admin/school/delSchoolTerm/'.$schoolTerm->id)}}" class="btn btn-xs btn-danger" onclick="return confirm('删除后无法找回,确定要删除吗?');">
