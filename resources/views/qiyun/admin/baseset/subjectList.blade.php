...
@section('content')
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <div class="nav-search" id="nav-search">
            ...
            <form class="form-search"  method="get" action="{{url('admin/baseset/subjectList')}}">
            
                <select name="flag" id="form-field-1">
                    <option value="0" @if($data->flag == 0) selected @endif>全部</option>
                    <option value="1" @if($data->flag == 1) selected @endif>学科名称</option>
                </select> 
...
@level(3)      
        ...
        <div class="">
            <form action="{{url('admin/baseset/addssubject')}}" method="post">
            @permission('base.create')
                  <input type="hidden" name="_token" value="{{ csrf_token() }}">
                  <a href="{{url('admin/baseset/addssubject')}}" class="a_tianjia">
        ...
        @permission('base.create')
        <div>
          <form action="{{url('admin/excel/subjectImport')}}" method="post" enctype="multipart/form-data" style="float:right;margin-top:-20px;">

                <input type="submit" class="btn btn-xs btn-info" value="导入学校学科信息" >
                <input type="file" name="excel" style="float: right;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
          </form>

          <a href="{{url('admin/excel/subjectTemplate')}}" class="btn btn-xs btn-info" style="float: right;margin-right: 10px;margin-top:-20px;">
        ...
        <form method="get" action="{{url('admin/baseset/subjectList')}}">
       <!-- 超级管理员 -->
...
@permission('base.modify')
                                                      <a href="{{url('admin/baseset/editsubject/'.$subject->id)}}" class="btn btn-xs btn-info">
                                                          <i class="icon-edit bigger-120"></i>
                                                      </a>
@endpermission
@permission('base.remove')
                                                      <a href="{{url('admin/baseset/delsubject/'.$subject->id)}}" style="width:29px" class="btn btn-xs btn-danger" onclick="return confirm('删除后将无法找回,确定要删除吗?');">
...
@endpermission
                         ...
                         {!! $data->appends(app('request')->all())->render() !!}  
                          <a href="{{url('admin/excel/subjectExport')}}" class="btn btn-xs btn-info" style="float: right;">
