...
@section('content')
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <div class="nav-search" id="nav-search">
            ...
            <form class="form-search"  method="get" action="{{url('admin/baseset/teachgroupList')}}">
                
                <select name="flag" id="form-field-1">
                    <option value="0" @if($data->flag == 0) selected @endif>全部</option>
                    <option value="1" @if($data->flag == 1) selected @endif>教研组名称</option>
                </select> 
...
@level(3)        
         ...
         <div class="">
            <form action="{{url('admin/baseset/addsteachgroup')}}" method="post">
            @permission('base.create')
                  <input type="hidden" name="_token" value="{{ csrf_token() }}">
                  <a href="{{url('admin/baseset/addsteachgroup')}}" class="a_tianjia">
        ...
        @permission('base.create')
        <div>
           <form action="{{url('admin/excel/teachImport')}}" method="post" enctype="multipart/form-data" style="float:right;margin-top:-20px;">

                <input type="submit" class="btn btn-xs btn-info" value="导入学校教研组信息" >
                <input type="file" name="excel" style="float: right;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
          </form>

          <a href="{{url('admin/excel/teachTemplate')}}" class="btn btn-xs btn-info" style="float: right;margin-right: 10px;margin-top:-20px;">
        ...
        <form method="get" action="{{url('admin/baseset/teachgroupList')}}">
       <!-- 超级管理员 -->
        ...
        <select style="float:left;margin:14px 0 0 10px;width: 110px;" id="teachgroup" name="teachgroup" >
              <option value="" >教研组</option>
               @if($data->labels['school'])
                    @foreach($data->labels['teachgroupinfo'] as $teachgroup)
                    <option class="teachgroup" value="{{$teachgroup->id}}" @if($data->labels['teachgroup'] == $teachgroup->id) selected @endif>{{$teachgroup->teachgroupName}}</option>
                    @endforeach
                @endif   
        </select>
        @endif
       ...
       <select style="float:left;margin:14px 0 0 10px;width: 110px;" id="teachgroup" name="teachgroup" >
              <option value="" >教研组</option>
               @foreach($data->teachgroup as $teachgroup)
                        <option value="{{$teachgroup->id}}" @if($data->labels['teachgroup'] == $teachgroup->id) selected @endif >{{$teachgroup->teachgroupName}}</option>
                @endforeach
        </select>
        @endif
                                  ...
                                  <form action="">
                                      @foreach($data as $teach)
                                          <tr>
                                              ...
                                              <td>{{$teach->organize_name}}</td>
                                              <td>{{$teach->city_name}}</td>
                                              <td>{{$teach->county_name}}</td>
                                              <td>{{$teach->schoolName}}</td>
                                              <td>{{$teach->teachgroupName}}</td>
                                              <td>{{$teach->status ? '激活':'锁定'}}</td>   
...
@permission('base.modify')
                                                      <a href="{{url('admin/baseset/editteachgroup/'.$teach->id)}}" class="btn btn-xs btn-info">
                                                          <i class="icon-edit bigger-120"></i>
                                                      </a>
@endpermission
@permission('base.remove')
                                                      <a href="{{url('admin/baseset/delteach/'.$teach->id)}}" style="width:29px" class="btn btn-xs btn-danger" onclick="return confirm('删除后将无法找回,确定要删除吗?');">
...
@endpermission
                       ...
                       {!! $data->appends(app('request')->all())->render() !!} 
                        <a href="{{url('admin/excel/teachExport')}}" class="btn btn-xs btn-info" style="float: right;">
...
<script language="javascript" type="text/javascript">
       ...
       $(function(){
            ...
            $('#teachgroup').html('<option value="">-所属教研组-</option>');
                // alert(cityid);
                $.ajax({
                    type:'get',
                    data:{'id':cityid},
            ...
            // alert(schoolid);
                $.ajax({
                    type:'get',
                    data:{'id':schoolid},
                    url:'{{url('admin/teachergroup/ajaxteachgroup')}}',
                    success:function(data){
                        // alert(data);
                        var str = '<option value="">--教研组--</option>';
                        $.each(data,function(i,val){
