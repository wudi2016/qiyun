...
@section('content')
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <div class="nav-search" id="nav-search">
            ...
            <form class="form-search"  method="get" action="{{url('admin/baseset/departmentList')}}">
                <select name="flag" id="form-field-1">
                    <option value="0" @if($data->flag == 0) selected @endif>全部</option>
                    <option value="1" @if($data->flag == 1) selected @endif>部门名称</option>
                </select> 
...
@level(3)   
        <div class="">
            <form action="{{url('admin/baseset/addsdepartment')}}" method="post">
            @permission('base.create')
                  <input type="hidden" name="_token" value="{{ csrf_token() }}">
                  <a href="{{url('admin/baseset/addsdepartment')}}" class="a_tianjia">
      ...
      @permission('base.create')
      <div>
           <form action="{{url('admin/excel/departmentImport')}}" method="post" enctype="multipart/form-data" style="float:right;margin-top:-20px;">

                <input type="submit" class="btn btn-xs btn-info" value="导入学校部门信息" >
                <input type="file" name="excel" style="float: right;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
          </form>

          <a href="{{url('admin/excel/departmentTemplate')}}" class="btn btn-xs btn-info" style="float: right;margin-right: 10px;margin-top:-20px;">
      ...
      @endpermission
        ...
        <form method="get" action="{{url('admin/baseset/departmentList')}}">
       <!-- 超级管理员 -->
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="table-responsive">
                                  ...
                                  <form action="">
                                      @foreach($data as $dep)
                                          <tr>
                                              ...
                                              <td>{{$dep->organize_name}}</td>
                                              <td>{{$dep->city_name}}</td>
                                              <td>{{$dep->county_name}}</td>
                                              <td>{{$dep->schoolName}}</td>
                                              <td>{{$dep->departName}}</td>
                                              <td>{{$dep->status ? '激活':'锁定'}}</td>
                                              ...
                                              <td>
                                                  <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                      ...
                                                      <a href="{{url('admin/baseset/editdepartment/'.$dep->id)}}" class="btn btn-xs btn-info">
                                                          <i class="icon-edit bigger-120"></i>
                                                      </a>
                                                      @endpermission
                                                      @permission('base.remove')
                                                      <a href="{{url('admin/baseset/deldep/'.$dep->id)}}" style="width:29px" class="btn btn-xs btn-danger" onclick="return confirm('删除后将无法找回,确定要删除吗?');">
                          ...
                          <a href="{{url('admin/excel/departmentExport')}}" class="btn btn-xs btn-info" style="float: right;">
