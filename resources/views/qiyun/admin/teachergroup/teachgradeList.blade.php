...
@section('content')
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <div class="nav-search" id="nav-search">
            ...
            <form class="form-search"  method="get" action="{{url('admin/teachergroup/teachgradeList')}}">
...
@if(\Auth::user()->level() > 2)
         ...
         <div class="">
            <form action="{{url('admin/teachergroup/addteachgrade')}}" method="post">
            @permission('teachgroup.create')
                  <input type="hidden" name="_token" value="{{ csrf_token() }}">
                  <a href="{{url('admin/teachergroup/addteachgrade')}}" class="a_tianjia">
        ...
        @permission('teachgroup.create')
      <div>
        <form action="{{url('admin/excel/teachgradeImport')}}" method="post" enctype="multipart/form-data" style="float:right;margin-top:-20px;">

                <input type="submit" class="btn btn-xs btn-info" value="导入年级分组信息" >
                <input type="file" name="excel" style="float: right;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
          </form>

          <a href="{{url('admin/excel/teachgradeTemplate')}}" class="btn btn-xs btn-info" style="float: right;margin-right: 10px;margin-top:-20px;">
...
@endif
        ...
        <form method="get" action="{{url('admin/teachergroup/teachgradeList')}}">
       <!-- 超级管理员 -->
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="table-responsive">
                                  ...
                                  <form action="">
                                      @foreach($data as $grade)
                                          <tr>
                                              ...
                                              <td>{{$grade->organize_name}}</td>
                                              <td>{{$grade->city_name}}</td>
                                              <td>{{$grade->county_name}}</td>
                                              <td>{{$grade->schoolName}}</td>
                                              <td>{{$grade->gradeName}}</td>
                                              <td>{{$grade->username}}</td>
                                              <td>{{$grade->realname}}</td>
                                              <td>{{$grade->status ? '激活':'锁定'}}</td>
                                              ...
                                              <td>
                                                  <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                      ...
                                                      <a href="{{url('admin/teachergroup/editteachgrade/'.$grade->id)}}" class="btn btn-xs btn-info">
                                                      ...
                                                      <a href="{{url('admin/teachergroup/delteachgrade/'.$grade->id)}}" style="width:29px" class="btn btn-xs btn-danger" onclick="return confirm('删除后将无法找回,确定要删除吗?');">
                         ...
                         <a href="{{url('admin/excel/teachgradeExport')}}" class="btn btn-xs btn-info" style="float: right;">
