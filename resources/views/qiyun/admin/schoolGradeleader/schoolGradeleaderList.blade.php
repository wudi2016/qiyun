...
@section('content')
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <div class="nav-search" id="nav-search">
            <form action="{{url('admin/school/schoolGradeleaderList')}}" method="get" class="form-search">
                <select name="type" id="form-field-1">
                    ...
                    <option value="4" @if($data->type == 4) selected @endif>年级组长</option>
    ...
    <div class="page-content">
        ...
        <a href="{{url('admin/school/addSchoolGradeleader')}}" class="btn btn-xs btn-info">
            <i class="icon-ok bigger-110">添加</i>
        </a>
        @endpermission

        @permission('posts.add')
        <form action="{{url('admin/excel/schoolGradeleaderImport')}}" method="post" enctype="multipart/form-data" style="float:right;">
            <input type="submit" class="btn btn-xs btn-info" value="导入年级组长信息">
            <input type="file" name="excel" style="float:right;">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
        </form>
        <a href="{{url('admin/excel/schoolGradeleaderTemplate')}}" class="btn btn-xs btn-info" style="float: right;margin-right: 10px;">
            下载导入模板
        </a>
        @endpermission
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="table-responsive">
                            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                ...
                                <tbody>
                                  <form action="" method="post">
                                      <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                      @foreach($data as $schoolGradeleader)
                                          <tr>

                                              <td>
                                                  <a href="#">{{$schoolGradeleader->id}}</a>
                                              </td>
                                              <td>{{$schoolGradeleader->schoolName}}</td>
                                              <td>{{$schoolGradeleader->gradeName}}</td>
                                              <td>{{$schoolGradeleader->realname}}</td>
                                              <td>{{$schoolGradeleader->status ? '激活':'锁定'}}</td>
                                              <td>{{$schoolGradeleader->created_at}}</td>
                                              <td>{{$schoolGradeleader->updated_at}}</td>
                                              ...
                                              <td>
                                                  <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                      ...
                                                      <a href="{{url('admin/school/editSchoolGradeleader/'.$schoolGradeleader->id)}}" class="btn btn-xs btn-info">
                                                          <i class="icon-edit bigger-120"></i>
                                                      </a>
                                                      @endpermission

                                                      @permission('posts.del')
                                                      <a href="{{url('admin/school/delSchoolGradeleader/'.$schoolGradeleader->id)}}" class="btn btn-xs btn-danger" onclick="return confirm('删除后无法找回,确定要删除吗?');">
                            ...
                            {{--<a href="{{url('admin/excel/schoolGradeleaderExport')}}" class="btn btn-xs btn-info" style="float:right;">--}}
                                {{--<i class="icon-ok bigger-110">导出年级组长信息</i>--}}
                            {{--</a>--}}
                            <form action="{{url('admin/excel/schoolGradeleaderExport')}}" method="post" style="float: right;">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <input type="hidden" name="export" value="{{$export}}">
                                <input type="submit" class="btn btn-xs btn-info" value="导出年级组长信息">
                            </form>
