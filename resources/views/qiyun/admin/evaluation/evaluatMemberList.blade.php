...
@section('content')
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <div class="nav-search" id="nav-search">
            <form action="{{url('admin/evaluation/evaluatMemberList')}}" method="get" class="form-search">
                <select name="type" id="form-field-1">
                    ...
                    <option value="3" @if($data->type == 3) selected @endif>用户名</option>
                                  ...
                                  <form action="">
                                      @foreach($data as $member)
                                          <tr>
                                              ...
                                              <td>{{$member->evaluatName}}</td>
                                              <td>{{$member->realname}}</td>
                                              <td class="hidden-480">{{$member->isManage ? '负责人' : '普通成员'}}</td>
                                              <td>{{$member->status ? '激活':'锁定'}}</td>
                                              ...
                                              <td>
                                                  <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                      ...
                                                      <a href="{{url('admin/evaluation/editEvaluatMember/'.$member->id)}}" class="btn btn-xs btn-info">
                                                          <i class="icon-edit bigger-120"></i>
                                                      </a>
                                                      {{--@endpermission--}}

                                                      {{--@permission('evaluation.del')--}}
                                                      <a href="{{url('admin/evaluation/delEvaluatMember/'.$member->id)}}" class="btn btn-xs btn-danger" onclick="return confirm('删除后将无法找回,确定要删除吗?');">
