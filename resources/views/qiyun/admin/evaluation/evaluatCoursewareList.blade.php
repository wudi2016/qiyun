...
@section('content')
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <div class="nav-search" id="nav-search">
            <form action="{{url('admin/evaluation/evaluatCourseware')}}" method="get" class="form-search">
                <select name="type" id="form-field-1">
                    ...
                    <option value="3" @if($data->type == 3) selected @endif>课件名称</option>
                        ...
                        <div class="table-responsive">
                            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                ...
                                <tr>
                                    ...
                                    <th>评课名称</th>
                                    <th>评课的课件名称</th>
                                    <th class="hidden-480">评课课件</th>
                                    <th class="hidden-480">课件格式</th>
                                    <th class="hidden-480">评课课件大小(k)</th>
                                    <th>创建时间</th>
                                    <th>修改时间</th>
                                    <th>操作</th>
                                </tr>
                                ...
                                <tbody>
                                  <form action="">
                                      @foreach($data as $evaluatCourseware)
                                          <tr>
                                              {{--<td class="center">--}}
                                                  {{--<label>--}}
                                                      {{--<input type="checkbox" class="ace" />--}}
                                              ...
                                              <td>
                                                  <a href="#">{{$evaluatCourseware->id}}</a>
                                              </td>
                                              <td>{{$evaluatCourseware->evaluatName}}</td>
                                              <td>{{$evaluatCourseware->evaluatCourseName}}</td>
                                              <td><a class="fancybox" href="{{asset($evaluatCourseware->evaluatPath)}}" data-width="640" data-height="360">查看</a></td>
                                              <td>{{$evaluatCourseware->evaluatFomat}}</td>
                                              <td>{{$evaluatCourseware->evaluatSize}}</td>
                                              <td>{{$evaluatCourseware->created_at}}</td>
                                              <td>{{$evaluatCourseware->updated_at}}</td>
                                              ...
                                              <td>
                                                  <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                      ...
                                                      <a href="{{url('admin/evaluation/editEvaluatCourseware/'.$evaluatCourseware->id)}}" class="btn btn-xs btn-info">
                                                          <i class="icon-edit bigger-120"></i>
                                                      </a>
                                                      {{--@endpermission--}}

                                                      {{--@permission('evaluation.del')--}}
                                                      <a href="{{url('admin/evaluation/delEvaluatCourseware/'.$evaluatCourseware->id)}}" class="btn btn-xs btn-danger" onclick="return confirm('删除后无法找回,确定要删除吗?');">
