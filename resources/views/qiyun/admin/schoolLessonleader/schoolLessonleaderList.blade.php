...
@section('content')
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <div class="nav-search" id="nav-search">
            <form action="{{url('admin/school/schoolLessonleaderList')}}" method="get" class="form-search">
                <select name="type" id="form-field-1">
                    ...
                    <option value="4" @if($data->type == 4) selected @endif>学科</option>
                    <option value="5" @if($data->type == 5) selected @endif>备课组长</option>
                    <option value="6" @if($data->type == 8) selected @endif>全部</option>
                </select>
    ...
    <div class="page-content">
        ...
        <a href="{{url('admin/school/addSchoolLessonleader')}}" class="btn btn-xs btn-info">
            <i class="icon-ok bigger-110">添加</i>
        </a>
        @endpermission

        @permission('posts.add')
        <form action="{{url('admin/excel/schoolLessonleaderImport')}}" method="post" enctype="multipart/form-data" style="float: right;">
            <input type="submit" class="btn btn-xs btn-info" value="导入备课组长信息">
            <input type="file" name="excel" style="float: right">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
        </form>
        <a href="{{url('admin/excel/schoolLessonleaderTemplate')}}" class="btn btn-xs btn-info" style="float: right;margin-right: 10px;">
            下载导入模板
        </a>
        @endpermission
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="table-responsive">
                            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                ...
                                <tbody>
                                  <form action="" method="post">
                                      <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                      @foreach($data as $schoolLessonleader)
                                          <tr>
                                              {{--<td class="center">--}}
                                                  {{--<label>--}}
                                                      {{--<input type="checkbox" class="ace" name="id[]" value="{{$schoolLessonleader->id}}"/>--}}
                                                      {{--<span class="lbl"></span>--}}
                                                  {{--</label>--}}
                                              {{--</td>--}}

                                              <td>
                                                  <a href="#">{{$schoolLessonleader->id}}</a>
                                              </td>
                                              <td>{{$schoolLessonleader->schoolName}}</td>
                                              <td>{{$schoolLessonleader->gradeName}}</td>
                                              <td>{{$schoolLessonleader->subjectName}}</td>
                                              <td>{{$schoolLessonleader->realname}}</td>
                                              <td>{{$schoolLessonleader->status ? '激活':'锁定'}}</td>

                                              <td>{{$schoolLessonleader->created_at}}</td>
                                              <td>{{$schoolLessonleader->updated_at}}</td>
                                              ...
                                              <td>
                                                  <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                      ...
                                                      <a href="{{url('admin/school/editSchoolLessonleader/'.$schoolLessonleader->id)}}" class="btn btn-xs btn-info">
                                                          <i class="icon-edit bigger-120"></i>
                                                      </a>
                                                      @endpermission

                                                      @permission('posts.del')
                                                      <a href="{{url('admin/school/delSchoolLessonleader/'.$schoolLessonleader->id)}}" class="btn btn-xs btn-danger" onclick="return confirm('删除后无法找回,确定要删除吗?');">
                            ...
                            {{--<a href="{{url('admin/excel/schoolLessonleaderExport')}}" class="btn btn-xs btn-info" style="float:right;">--}}
                                {{--<i class="icon-ok bigger-110">导出备课组长信息</i>--}}
                            {{--</a>--}}
                            <form action="{{url('admin/excel/schoolLessonleaderExport')}}" method="post" style="float: right;">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <input type="hidden" name="export" value="{{$export}}">
                                <input type="submit" class="btn btn-xs btn-info" value="导出备课组长信息">
                            </form>
        ...
        $('#schoolgrade').change(function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    $.each(data,function(i,val){
                        str += '<option class="subject" value="'+val.id+'">'+val.subjectName+'</option>';
                    })
