 ...
 */
primeApp.controller('subjectAudioCtrl',function($scope,$location,myHttp){

    // 获取地址中的参数
    $scope.url = $location.absUrl();
    $scope.urlarr = $scope.url.split('/');
    $scope.id = $scope.urlarr[$scope.urlarr.length - 1];
    $scope.audioReset = function(id){
    ...
    $scope.insert = function(){
        ...
        myHttp.postData('/research/insertSubjectAudio',$scope.postdata).success(function(response){
