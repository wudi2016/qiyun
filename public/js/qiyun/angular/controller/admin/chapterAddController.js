primeApp.controller("chapterAddController", ["$scope","myHttp", function ($scope,myHttp) {
    ...
    $scope.post = function(){
    	...
    	if(!$scope.postdata.resourceSection){
		    ...
		    myHttp.postData('/admin/resource/chapterDoAdd',$scope.postdata).success(function (response) {

		        if(response == 1){
		        	// alert('添加成功！');
		        	if(!confirm('添加成功，是否继续添加？'))
                    window.location.href = '/admin/resource/chapterList';
		        }else{
		        	alert('添加失败！');
		        }
