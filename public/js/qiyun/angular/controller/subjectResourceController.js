 ...
 */
primeApp.controller('subjectResourceCtrl',function($scope,$location,myHttp){

    // 获取地址中的参数
    $scope.url = $location.absUrl();
    $scope.urlarr = $scope.url.split('/');
    $scope.id = $scope.urlarr[$scope.urlarr.length - 1];
    $scope.resourceReset = function(id){
    ...
    $scope.insert = function(){
        ...
        myHttp.postData('/research/insertSubjectResource',$scope.postdata).success(function(response){
            if(response.status == '1'){
                location.href = '/research/subjectInfo/' + $scope.id;
