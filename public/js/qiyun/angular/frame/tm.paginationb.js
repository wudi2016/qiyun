 ...
 * Version: 1.0.0
 */
angular.module('tm.paginationb', []).directive('tmPaginationb',[function(){
    return {
        restrict: 'EA',
        ...
        //         '第<input type="text" ng-model="jumpPageNum" class="dijiye"  ng-keyup="jumpToPage($event)"/>页 ' +
        ...
        //     '</div>',
        template: '<div class="page-list">' +
            '<div class="pagination" ng-show="conf.totalItems > 0">' +
                '<div class="pg" ng-class="{disabled: conf.currentPage == 1}" ng-click="prevPage()"><span>&laquo;</span></div>' +
                '<div class="pg" ng-repeat="item in pageList track by $index" ng-class="{pg_n: item == conf.currentPage, separate: item == \'...\'}" ' + 'ng-click="changeCurrentPage(item)">' + '<span>{{ item }}</span>' +  '</div>' +
                '<div class="pg" ng-class="{disabled: conf.currentPage == conf.numberOfPages}" ng-click="nextPage()"><span>&raquo;</span></div>' +
            '</div>' +
            '<div class="page-total" ng-show="conf.totalItems > 0">' +
                '第<input type="text" style="text-align:center;" ng-model="jumpPageNum" class="dijiye"  ng-keyup="jumpToPage($event)"/>页  &nbsp;&nbsp;' +
